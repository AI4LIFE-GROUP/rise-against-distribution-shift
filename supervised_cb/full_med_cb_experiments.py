"""
Code for running IPW and DRO on synthetic dataset with two covariates, one treatment, and outcome.

python full_med_cb_experiments.py --filename toylogpol_testshift --alpha 0.8
"""
...
parser.add_argument('--outdir', default='saved_results/cb/')
# parser.add_argument('--filename', default='toygaussian')
parser.add_argument('--filename', default='toylogpol')
parser.add_argument('--alpha', default=0.8, type=float)
parser.add_argument('--num_runs', default=5, type=int)
args = parser.parse_args()

default_args_dict = {"steps":100, "dlr":.005, "ntrain":2000, "ndim":2, "alpha":args.alpha, "kdual":2.0, "joint":False, "lip":1.0, "penalty":0.0,
                    "min_p":0.025, "model_type":"baseline", "turk":False, "oracle":False, "shuffle":0.0, "subcov":False,
                    "outdir":args.outdir, "ntest":20000, "sigma_x":1, "sigma_y":0.1, "nonsep":None, "filename":args.filename,
...
def subcovariate_shift_dro_eval(shifts):
    args_dict = default_args_dict.copy()
...
def covariate_shift_dro_eval(shifts):
    args_dict = default_args_dict.copy()
    args_dict["model_type"] = "dro_eval"
    args_dict["dlr"] = 0.5
    all_acc = []
...
def single_run(default_args_dict, shifts, run_id):
    file_prefix = default_args_dict["outdir"]+"/"+default_args_dict["filename"]+"_r_"+str(run_id)
                    # +"_a_"+str(default_args_dict["alpha"])+"_p_"+str(default_args_dict["penalty"])
    default_args_dict["run_id"] = run_id
    no_transfer_eval_acc, no_transfer_eval_loss, no_transfer_eval_params = no_transfer_eval(shifts)
    is_eval_acc, is_eval_loss, is_eval_params = is_eval(shifts)
    wis_eval_acc, wis_eval_loss, wis_eval_params = wis_eval(shifts)
    joint_dro_eval_acc, joint_dro_eval_loss, joint_dro_eval_params = joint_dro_eval(shifts)
    covariate_shift_dro_eval_acc, covariate_shift_dro_eval_loss, covariate_shift_dro_eval_params = covariate_shift_dro_eval(shifts)
    subcovariate_shift_dro_eval_acc, subcovariate_shift_dro_eval_loss, subcovariate_shift_dro_eval_params = subcovariate_shift_dro_eval(shifts)
    oracle_eval_acc, oracle_eval_loss, oracle_eval_params = oracle_eval(shifts)
...
def plot_confidence(default_args_dict, shifts):
    ...
    for run_id in range(default_args_dict["num_runs"]):
        ...
        file_prefix = default_args_dict["outdir"]+"/"+default_args_dict["filename"]+"_a"+str(default_args_dict["alpha"])+"_r_"+str(run_id)
                    # +"_p_"+str(default_args_dict["penalty"])
        print("Opening", file_prefix)
    ...
    ax.set_xticks(ind)
    ax.set_xticklabels([np.round(a/0.8, decimals=2) for a in shifts], rotation=35)
    ...
    rects1 = ax.errorbar(ind, mean_algo_loss_test[0,:], yerr=std_algo_loss_test[0,:],
                    linewidth=10, alpha=0.5, color="lightseagreen", marker='X', label='Standard')
    ...
    rects4 = ax.errorbar(ind, mean_algo_loss_test[3,:], yerr=std_algo_loss_test[3,:],
                    linewidth=10, alpha=0.5, marker='o', color="olive", label='JointDRO')
    ...
    rects6 = ax.errorbar(ind, mean_algo_loss_test[5,:], yerr=std_algo_loss_test[5,:],
                    linewidth=10, alpha=0.5, marker='D', color="mediumvioletred", label='RISe')
    # rects7 = ax.errorbar(ind, mean_algo_loss_test[6,:], yerr=std_algo_loss_test[6,:],
    #                 linewidth=10, alpha=0.5, color="cornflowerblue", label='Oracle')
