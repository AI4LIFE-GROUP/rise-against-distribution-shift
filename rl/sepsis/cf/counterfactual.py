...
"""
import numpy as np
from sepsis.mdptoolboxSrc import mdp as mdptools
import warnings
from sepsis.cf import gumbelTools as gt
...
class MatrixMDP(object):
    ...
    def robustValueIteration(self, discount=0.99, epsilon=0.01, skip_check=False,
            robust_solver=None, max_iter=1000):
        """Calculate the value function of the optimal policy 
        for the marginal tx_mat and r_mat, using value iteration from pymdptoolbox
        ...
        # Run Policy Iteration
        pi = mdptools.RobustValueIteration(
            tx_mat_obs, r_mat_obs, discount=discount, skip_check=skip_check,
            robust_solver=robust_solver, max_iter=max_iter, epsilon=epsilon)
        pi.setSilent()
        pi.run()

        return pi.V
    
    def robustPolicyEvaluation(self, discount=0.9, obs_pol=None, skip_check=False,
            robust_solver=None, eval_type=1):
        """Calculate the optimal policy for the marginal tx_mat and r_mat,
        using policy iteration from pymdptoolbox
        ...
        # Run Policy Iteration
        pi = mdptools.RobustPolicyIteration(
            tx_mat_obs, r_mat_obs, discount=discount, skip_check=skip_check,
            policy0=obs_pol, robust_solver=robust_solver, eval_type=eval_type)
        pi.setSilent()
        pi._evalPolicyIterative()

        return pi.V
    
    def robustPolicyIteration(self, discount=0.9, obs_pol=None, skip_check=False,
            robust_solver=None, eval_type=1):
        """Calculate the optimal policy for the marginal tx_mat and r_mat,
        using policy iteration from pymdptoolbox
